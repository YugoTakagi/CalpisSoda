...
class target_value_class(object):
    ...
    def making_target_value_test_blue(self):
        ...
        return npLOB, LOB,npNEW_LOB, NEW_LOB
    def making_target_value_test_read(self):
        #bez = bezier(80)
        #bez5 = bezier(300)
        ...
        list_of_bezier_set2 = np.array([[-1*(1.725+ssp[0]),2.000+ssp[1]],[-1*(1.725+ssp[0]),2.500+ssp[1]],[-1*(0.775+ssp[0]),3+ssp[1]],[-1*(0.775+ssp[0]),3.5+ssp[1]]], dtype=np.float)
        list_of_bezier_set3 = np.array([[-1*(0.775+ssp[0]),3.5+ssp[1]],[-1*(0.775+ssp[0]),4+ssp[1]],[-1*(1.725+ssp[0]),4.5+ssp[1]],[-1*(1.725+ssp[0]),5+ssp[1]]], dtype=np.float)
